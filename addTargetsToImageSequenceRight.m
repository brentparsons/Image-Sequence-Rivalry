function imageSequenceTargetRight = addTargetsToImageSequenceRight(imageSequence, nImageBs, nRepsB, nTargets, targetImage)
...
% adds targets for the right eye
[i] = find(sortedSetStack3DTarget == targetImage);
toReplaceTargetRight = sortedSetStackReplaced
toReplaceTargetRight(1:2:end) = targetImage

for c = 1:length(i)
    sortedSetStack3DTarget(i(c))= toReplaceTargetRight(c);
end
...
imageSequenceTargetRight = reshape(setStackTarget', length(imageSequence), 1)
